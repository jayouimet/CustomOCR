...
class Crnn_64_512:
  ...
  _WEIGHTS_PATH = "ocr_crnn_64_512.weights.h5"

  def __init__(self, alphabet, verbose = False):
    inp = layers.Input(shape=(64,512,1), name="image")
    x = layers.Conv2D(32, 3, activation="relu", padding="same")(inp)
    x = layers.MaxPool2D((2,2))(x)  # now 32×256
    x = layers.Conv2D(64, 3, activation="relu", padding="same")(x)
    x = layers.MaxPool2D((2,2))(x)  # now 16×128
    # collapse height dimension:
    x = layers.Permute((2,1,3))(x)      # shape: (width=128, height=16, channels)
    x = layers.Reshape((128, 16*64))(x)   # sequence length=128 time steps
    # recurrent:
